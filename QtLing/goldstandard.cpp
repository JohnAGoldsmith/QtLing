...
GoldStandard::GSMap* GoldStandard::m_clone_GS_map() const
{
    GSMap* newmap = new GSMap();
    for (GSMap::iterator gsIter = m_GS->begin();
         gsIter != m_GS->end(); gsIter++) {
...
void GoldStandard::m_delete_GS_map()
{
    if (m_GS) {
        for (GSMap::iterator gsIter = m_GS->begin();
             gsIter != m_GS->end(); gsIter++) {
...
void GoldStandard::add_parse_triple(QString &word, QString &stem, QString &affix)
{
    GSMap::iterator wordIter = m_GS->find(word);
    if (wordIter == m_GS->end()) {
        qDebug() << QString("GoldStandard::add_parse_triple: word") << word << QString("not found in m_GS");
        return;
    }
...
void GoldStandard::m_openXML(MainWindow* p_main_window)
{
...
/* old code
    ...
    --THIS FUNCTIONALITY IS MOVED INTO m_openXML()
*/
...
//				  QString( errorMsg + "\nLine: %1" + "Col: %2" ).arg( errorLine ).arg( errorColumn ), QMessageBox::Ok, NULL, NULL );
                            ...
                            while( !node4.isNull() && node4.isElement() && node4.nodeName() == "piece" )
                            {
                                ...
                                if( morph.tagName() == "affix" || morph.tagName() == "root" )
                                {
                                    ...
                                    if (start != 0) {
                                        ...
                                        add_parse_triple(word, stem, affix);
                                    }

                                    lastEnd = start + pieceLength - 1;
    ...
    {
        qDebug() << QString("GoldStandar::m_parse_XML: XML file not set");
        return;
    }
