...
/*!
 * \brief Evaluate the output generated by Linguistica (stored in a
 * CWordCollection object) by comparing it with the parses stored in the
 * ParseMap of an instance of a GoldStandard object.
 * \param p_word_collection
 * \return True if evaluation succeeded, false if an error occurred.
 */
bool GoldStandard::evaluate(CWordCollection *p_word_collection)
{
    ...
    m_true_positive_parses = ParseMapHandler();
    m_retrieved_parses = ParseMapHandler();
    ...
    for (gs_pm_iter = m_gs_parses->constBegin();
        ...
        for (gs_ptm_iter = p_gs_ptm->constBegin();
            ...
            if (p_lxa_ptm->contains(stem)) {
                ...
                m_true_positive_parses.add_parse_triple(str_word, stem, suffix); // **** NEED TO CHANGE **** //
        ...
        for (lxa_ptm_iter = p_lxa_ptm->constBegin();
            ...
            if (stem != str_word) {// only count parses with non-null suffixes
                ...
                m_retrieved_parses.add_parse_triple(str_word, stem, suffix); // **** NEED TO CHANGE **** //
...
{
    ParseMapHandler eval_parses = p_eval->m_parses;  //**** NEED TO CHANGE ****//
...
//				  QString( errorMsg + "\nLine: %1" + "Col: %2" ).arg( errorLine ).arg( errorColumn ), QMessageBox::Ok, NULL, NULL );
    ...
    if( node1.isNull() || !node1.isElement() || node1.nodeName() != "word-list" ) {
               ...
               node2.nodeName() == "word" )
        {
            ...
            newWord = false;
            ParseMap* p_parsemap = m_gs_parses.get_map();
            wordIter = p_parsemap->find(word);

            if (wordIter == p_parsemap->end()) {
                wordIter = p_parsemap->insert(word, new Parse_triple_map());
                newWord = true;
            }
                    ...
                    while( !node4.isNull() && node4.isElement() && node4.nodeName() == "piece" ) {
                        ...
                        if ( morph.tagName() == "affix" || morph.tagName() == "root" ) {
                            ...
                            if (start != 0) {
                                ...
                                m_gs_parses.add_parse_triple(word, stem, affix);
                                if ( start < lastEnd ) {
