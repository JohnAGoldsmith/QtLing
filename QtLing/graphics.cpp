...
void decagon(CSignature* pSig, int x, int y, int row_delta, lxa_graphics_scene * scene, int count)
{
    ...
    QRectF bR6 = p_text_item1->sceneBoundingRect();
    QRectF bR7 = p_text_item2->sceneBoundingRect();
    QRectF bR8 = p_text_item3->sceneBoundingRect();
    QRectF bR9 = p_text_item4->sceneBoundingRect();
    QRectF bR10 = p_text_item5->sceneBoundingRect();
    p_text_item6->setPos( x+3  - bR6.width()/2, y-35 - bR6.height()   );
    p_text_item7->setPos( x-35 - bR7.width()  , y-12 - bR7.height()/2 );
