...
LowerTableView::LowerTableView(MainWindow * window)
{
   ...
   QFont sansFont("Ariel", 12); // was 20 John Goldsmith August 2019
   setFont(sansFont);
...
{
    eDataType                  UpperView_data_type = m_parent_window->m_tableView_upper_1->get_data_type();
    ...
    case  e_data_signatures_graph_edges:{
        ...
        sig_pair*             this_edge = this_lexicon->get_suffixal_sig_pairs()->get_map()->value(edge_key);
...
void LowerTableView::graphics_sig_graph_edges(CSignature* pSig, CLexicon* p_lexicon)
{
    ...
    QMap<QString,sig_pair*>::iterator edge_iter = p_lexicon->get_suffixal_sig_pairs()->get_map()->begin();
    while (edge_iter !=  p_lexicon->get_suffixal_sig_pairs()->get_map()->constEnd()){
...
void LowerTableView::table_passive_signature(CSignature *p_this_sig)
{
    ...
    QMap<QString, sig_pair*> * pMap = get_lexicon()->get_suffixal_sig_pairs()->get_map();
    ...
    while (this_sig_graph_edge_iter.hasNext()){
        ...
        if (p_this_sig == p_edge->m_sig_1){
            ...
            Morphs[p_edge->m_sig_2] = p_edge->m_stem_difference;
        }
    }
